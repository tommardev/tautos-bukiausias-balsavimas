...
2. Open the project in Antigravity IDE. Verify that required MCP servers (Context7, Sonatype) are configured in ~/.gemini/config/mcp_config.json or under Additional Options (...) > MCP Servers. Never commit keys.
...
## Antigravity IDE & MCP Configuration (once per machine)

Antigravity uses native MCP servers configured in `~/.gemini/config/mcp_config.json` or loaded via plugins in `~/.gemini/config/plugins/`:
...
| `.agents/rules/ai-coding-native-rules.md`  | Antigravity workspace workflow + tools (`trigger: always_on`) ger: always_on) |
| `GEMINI.md`                                | Antigravity native project instructions pointer              |
